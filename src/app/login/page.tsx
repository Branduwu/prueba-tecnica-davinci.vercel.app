...
export default function LoginPage() {
  ...
  return <main className="login"><section><p className="eyebrow">MERCADO CENTRAL</p><h1>Tu operación, en orden.</h1><p>{demo ? 'Evalúa el ERP sin servicios externos. Los datos del modo demo se guardan en este navegador.' : 'Accede al punto de venta y controla cada movimiento del negocio.'}</p></section><form onSubmit={submit} className="card login-card"><h2>Iniciar sesión</h2>{demo && <><p className="demo-label">MODO DEMO</p><section className="demo-access" aria-label="Acceso de demostración"><h3>Acceso de demostración</h3><p className="muted">Credenciales exclusivas del entorno de evaluación.</p><div className="demo-actions"><button type="button" className="secondary" onClick={() => fillDemo('admin')}>Usar administrador demo</button><button type="button" className="secondary" onClick={() => fillDemo('cashier')}>Usar cajero demo</button></div></section></>}<label>Correo<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="nombre@supermercado.com" /></label><label>Contraseña<input type="password" required value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <p className="error">{error}</p>}<button disabled={loading}>{loading ? 'Ingresando…' : 'Iniciar sesión'}</button></form></main>
}
